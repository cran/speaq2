...
#' @param type the operations to be performed, this can be multiple and are performed sequantially. Any of 'unit', 'pareto', 'log10', 'log2', 'center', 'range', 'vast' or 'prob.Q' are accepted.
...
SCANT <- function(data.matrix, type = "unit", what = "columns") {
    ...
    for (N in 1:length(type)) {
        if (!(type[N] %in% c("unit", "pareto", "log10", "log2", "center", "range", "vast", "prob.Q"))) {
