#' Realignment with hierarchical clustering (used in the PeakAligner function)
#'
#' Internal function in the PeakAligner function for generating the hierarchical clustering tree and cutting it.
#'
#' @param current.peaks A number of neighbouring peaks to be aligned.
...
hclust.grouping <- function(current.peaks, min.samp.grp = 1, max.dupli.prop = 0.25, maxClust = 10, linkage = "average") {
    ...
    if (nrow(current.peaks) <= 1) {
        ...
        if (length(grps) > 0) {
            ...
            #### will usually not be the case) If there is an overlap, do an extensice search to find it overlap
            #### check steps: sort the starting values from low to hich and sort the stop values acording to the
